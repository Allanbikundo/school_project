...
public class GenericSource extends Source {
	 ...
	 * @param intArrDstr The packet interarrival time (sec)
	 * @param rate The packet generation rate (packets/sec)
	 * @param pktLngthDstr The packet length distribution c for constant, u for uniform e for exponential, p for pareto, l for lognormal
	 * @param pktLength The packet length (bits)
	 * @throws UnknownDistributionException 
	 */
	public GenericSource(int sourceId,char intArrDstr, float rate, char pktLngthDstr,float pktLength) throws UnknownDistributionException {
		String msg=null;
		if((pktLngthDstr!='c')&&(pktLngthDstr!='e')&&(pktLngthDstr!='u')){
			msg = "\'"+pktLngthDstr+"\' is not a valid distribution for packet length. Only \'c\',\'u\' and \'e\' are allowed.";
			throw new UnknownDistributionException(msg);
		}
		else if((intArrDstr!='c')&&(intArrDstr!='e')&&(intArrDstr!='u')){
			msg="\'"+intArrDstr+"\' is not a valid distribution for packet interarrival time. Only \'c\',\'u\' and \'e\' are allowed.";
			throw new UnknownDistributionException(msg);
		}
		else{
			_sourceId = sourceId;
	...
	public void getNextPacket() {
		if (session.isEmpty()) {
			session.addElement(new VirtualPacket((int)genRv(_pktLngthDstr,_avgPktLength,Float.MAX_VALUE,1f), (int)genRv(_intArrDstr, _avgIntArr,Float.MAX_VALUE,1f), 1));
		}
